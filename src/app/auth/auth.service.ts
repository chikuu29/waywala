...
export class AuthService {
  ...
  public authentication( name: string, email: string,phone_no: string, isLogin: boolean, role: String, _refreshkey: any, expiration_date: any,user_profile_image:any) {
    var userInfo = new user(name, email,phone_no, isLogin, role, _refreshkey, expiration_date,user_profile_image)
  ...
  public autoSignIn() {
    ...
    if (new Date(authInfo.expiration_date).getTime() > new Date().getTime()) {
      ...
      this.authentication(authInfo.name, authInfo.email,authInfo.phone_no, true, authInfo.role, authInfo._refreshkey, authInfo.expiration_date,authInfo.user_profile_image)
